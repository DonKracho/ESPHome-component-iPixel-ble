...
<img src="docs/LEDPixelBoard.jpg" alt="LED Pixel Board" width="200">
...
- **Known issues:**  
  ...
  - The yaml file has some internal comtonemts because they are not used and implemented right now. Deleting these may cause compilation erros!
  - There has been a deep copy issue with huge strings runing out of memory. I did not check the entire code for unnecesary deeep copies of huge data.
...
- **Sending PNG and animated GIFs**  
  Unfortunately the text (data) component is linited to 255 bytes. Using the max_length parameter did not apply. I need to figure out how to get a huge amount of data into the ESPHome device. A http request using an URL pointing an image source or reenabling the REST api may be a suggestion.
...
- **reverse engeneering of the play program feature**  
  A nice feature to queue inmage and text effects.

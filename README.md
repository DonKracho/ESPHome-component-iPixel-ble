...
- **Known issues:**  
  ...
  - The yaml file has some internal components because. They are pepared, but not used and implemented right now. Deleting these may cause compilation erros!
  - There has been a deep copy issue with huge strings runing out of memory. I did not check the entire code for unnecesary deep copies of big data.

- **Getting the display matrix size from the device itself**  
  Setting a fixed size isn't that smart;) The IPixel Color App is able to identify the display size.
